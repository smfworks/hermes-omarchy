...
- `install --no-ollama` / `--no-autostart` — opt out per part
- `check` — verify current state, exit 0/1 (scriptable health probe)
- `remove` — revert autostart; `remove --all` also disables the ollama service
...
- **W1** — Omarchy agent skill symlinked for Hermes (`~/.hermes/skills`)
